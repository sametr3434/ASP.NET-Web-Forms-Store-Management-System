            ...
            using (var tr = b.Baglanti.BeginTransaction())
            {
                try
                {
                    ...
                    foreach (DataRow row in sepet.Rows)
                    {
                        ...
                        using (var cmd = new SqlCommand(@"
                            INSERT INTO SiparisDetay (SiparisId, UrunId, UrunAdi, Beden, Renk, Adet, BirimFiyat, ToplamFiyat, ParaBirimi)
                            VALUES (@Sid, @Uid, @UrunAdi, NULL, NULL, @Adet, @Birim, @Toplam, N'TRY')", b.Baglanti, tr))
                        {
