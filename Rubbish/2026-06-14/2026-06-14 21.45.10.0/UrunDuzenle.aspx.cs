...
namespace Saat.Saat0604
{
    public partial class UrunDuzenle : Page
    {
        ...
        protected void btnSatisOzeti_Click(object sender, EventArgs e)
        {
            int urunId = Convert.ToInt32(lblUrunID.Text);
            BaglantiBilgileri b = new BaglantiBilgileri();
            using (SqlCommand com = new SqlCommand(@"
                SELECT CONVERT(date, s.SiparisTarihi) AS Tarih,
                       sd.Beden AS kasano, sd.Renk, SUM(sd.Adet) AS Adet
                FROM SiparisDetay sd
                INNER JOIN Siparis s ON sd.SiparisId = s.ID
                WHERE sd.UrunId = @UrunId
                GROUP BY CONVERT(date, s.SiparisTarihi), sd.Beden, sd.Renk
                ORDER BY CONVERT(date, s.SiparisTarihi) DESC", b.Baglanti))
            {
                com.Parameters.AddWithValue("@UrunId", urunId);
                if (com.Connection.State == ConnectionState.Closed) com.Connection.Open();
                DataTable dt = new DataTable();
                ...
                if (dt.Rows.Count == 0)
                {
                    pnlSatisYok.Visible = true;
                    rptSatisOzet.DataSource = null;
                    rptSatisOzet.DataBind();
                }
                else
                {
                    pnlSatisYok.Visible = false;
                    rptSatisOzet.DataSource = dt;
                    rptSatisOzet.DataBind();
                }
            }
            ScriptManager.RegisterStartupScript(this, GetType(), "satisModal", "$('#satisModal').modal('show');", true);
        }
    }
}
