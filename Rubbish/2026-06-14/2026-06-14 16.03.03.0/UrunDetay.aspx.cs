...
namespace Saat
{
    public partial class UrunDetay : System.Web.UI.Page
    {
                ...
                "SELECT Resim FROM UrunResim WHERE UrunId=@ID ORDER BY Siralama, ID", b.Baglanti))
            {
                ...
                using (var dr = com.ExecuteReader())
                {
                    while (dr.Read())
                    {
                        ...
                        if (!string.IsNullOrEmpty(r) && !resimler.Contains(r, StringComparer.OrdinalIgnoreCase))
                            resimler.Add(r);
                    }
                }
            }
        ...
        private void VaryantlariYukle(int urunId)
        {
            ...
            if (ddlKasa.Items.Count == 2)
                ddlKasa.SelectedIndex = 1;
            if (ddlRenk.Items.Count == 2)
