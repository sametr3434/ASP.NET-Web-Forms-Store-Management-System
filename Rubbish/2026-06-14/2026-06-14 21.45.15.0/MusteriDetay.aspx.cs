...
namespace Saat.Saat0604
{
    public partial class MusteriDetay : Page
    {
        ...
        private void SiparisleriGetir()
        {
            BaglantiBilgileri b = new BaglantiBilgileri();
            using (SqlCommand com = new SqlCommand(
                "SELECT ID, SiparisNo, SiparisTarihi, ToplamTutar, OdemeDurumu, SiparisDurumu, ISNULL(ParaBirimi,N'TRY') AS ParaBirimi FROM Siparis WHERE MusteriId=@ID ORDER BY SiparisTarihi DESC", b.Baglanti))
            {
                com.Parameters.AddWithValue("@ID", _musteriId);
                if (com.Connection.State == ConnectionState.Closed) com.Connection.Open();
        ...
        private void SepetiGetir()
        {
            ...
            using (SqlCommand com = new SqlCommand(
                @"SELECT s.ID, s.Adet, s.EklenmeTarihi, u.UrunAdi,
                         ISNULL(s.BirimFiyat, 0) AS BirimFiyat, ISNULL(s.ParaBirimi, N'TRY') AS ParaBirimi
                  FROM Sepet s
        ...
        private void OdemeGecmisiniGetirInner()
        {
            ...
            using (SqlCommand com = new SqlCommand(
                @"SELECT o.ID, s.SiparisNo, o.OdemeTarihi, o.KayitTarihi, o.Tutar, o.OdemeDurumu,
                         o.TutarTRY, o.TutarOrijinalPB, o.ParaBirimi
                  FROM OdemeKaydi o
