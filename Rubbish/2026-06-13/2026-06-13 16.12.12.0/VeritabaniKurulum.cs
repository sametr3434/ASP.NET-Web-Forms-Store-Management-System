...
FROM Urun WHERE Aktif=1 AND UrunAdi<>@ad ORDER BY ID DESC", conn))
            ...
            DemoSiparisEkle(conn, demoId, "SP202601150001", DATEADD_MONTHS(-2),
                urun1Ad, urun1Fiyat, 1, "Teslim Edildi", "Ödendi - Kapıda Ödeme");
            DemoSiparisEkle(conn, demoId, "SP202605280002", DATEADD_MONTHS(0).AddDays(-14),
                urun2Ad, urun2Fiyat, 2, "Kargoda", "Ödendi - EFT/Havale");
        }
        ...
        static void DemoAdresleriHazirla(SqlConnection conn, int demoId)
        {
            int adresSayisi;
            using (var cmd = new SqlCommand(
                "SELECT COUNT(1) FROM MusteriAdres WHERE MusteriId=@id", conn))
            {
                cmd.Parameters.AddWithValue("@id", demoId);
                adresSayisi = (int)cmd.ExecuteScalar();
            }
            if (adresSayisi > 0) return;

            using (var cmd = new SqlCommand(@"
INSERT INTO MusteriAdres (MusteriId, Baslik, Ad, Soyad, Telefon, Adres, Il, Ilce, PostaKodu, Varsayilan)
VALUES
(@mid, N'Ev', N'Demo', N'Kullanıcı', N'0555 123 45 67', N'Atatürk Cad. No:12 D:4', N'İstanbul', N'Kadıköy', N'34710', 1),
(@mid, N'İş', N'Demo', N'Kullanıcı', N'0555 123 45 67', N'Levent Plaza Kat:8', N'İstanbul', N'Beşiktaş', N'34330', 0);", conn))
            {
