    ...
    public static class MusteriAuth
    {
        ...
        public static int GirisYap(string eposta, string sifre)
        {
            ...
            using (var cmd = new SqlCommand(
                "SELECT ID, Sifre, Aktif FROM Musteri WHERE EPosta=@ep", b.Baglanti))
            {
        ...
        public static int Kayit(string ad, string soyad, string eposta, string telefon, string sifre)
        {
            string hash = HashOlustur(sifre);
            var b = new BaglantiBilgileri();
            using (var cmd = new SqlCommand(
                @"INSERT INTO Musteri(Ad,Soyad,EPosta,Telefon,Sifre,KayitTarihi,Aktif)
                  VALUES(@ad,@soyad,@ep,@tel,@sifre,@kt,1);
