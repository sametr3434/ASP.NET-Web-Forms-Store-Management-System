            ...
            using (var tr = b.Baglanti.BeginTransaction())
            {
                try
                {
                    ...
                    foreach (DataRow row in sepet.Rows)
                    {
                        ...
                        using (var cmd = new SqlCommand(@"
                            INSERT INTO SiparisDetay (SiparisId, UrunId, UrunAdi, Beden, Renk, Adet, BirimFiyat, ToplamFiyat)
                            VALUES (@Sid, @Uid, @UrunAdi, @Beden, @Renk, @Adet, @Birim, @Toplam)", b.Baglanti, tr))
                        {
                            cmd.Parameters.AddWithValue("@Sid", siparisId);
                            cmd.Parameters.AddWithValue("@Uid", urunId);
                            cmd.Parameters.AddWithValue("@UrunAdi", urunAdi);
                            cmd.Parameters.AddWithValue("@Beden", string.IsNullOrEmpty(kasa) ? (object)DBNull.Value : kasa);
