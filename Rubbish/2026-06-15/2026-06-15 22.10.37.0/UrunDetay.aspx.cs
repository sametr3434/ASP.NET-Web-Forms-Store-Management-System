...
namespace Saat
{
    public partial class UrunDetay : System.Web.UI.Page
    {
        ...
        private DataTable VaryantTablosuAl(int urunId)
        {
            ...
            using (var com = new SqlCommand(@"
                SELECT v.ID, v.KasaId, v.RenkId, v.StokAdedi, ISNULL(v.FiyatFarki, 0) AS FiyatFarki,
                       kb.kasano, r.RenkAdi, ISNULL(r.RenkKodu, N'#999999') AS RenkKodu
                FROM UrunVaryant v
        ...
        private void VaryantlariYukle(int urunId, string seciliKasaId, string seciliRenkId)
        {
            ...
            bool kasaVar = ddlKasa.Items.Count > 1;
            bool renkVar = ddlRenk.Items.Count > 1;
            pnlKasaGrup.Visible = kasaVar;
            pnlRenkGrup.Visible = renkVar;
            ViewState["KasaGerekli"] = kasaVar;
            ViewState["RenkGerekli"] = renkVar;
        ...
        private void SeciliVaryantiUygula()
        {
            ...
            {
                var tek = dt.Rows[0];
                UygulaVaryantSatir(tek);
                return;
            }

            bool kasaGerek = ViewState["KasaGerekli"] != null && Convert.ToBoolean(ViewState["KasaGerekli"]);
            bool renkGerek = ViewState["RenkGerekli"] != null && Convert.ToBoolean(ViewState["RenkGerekli"]);

            int? kasaId = kasaGerek && !string.IsNullOrEmpty(ddlKasa.SelectedValue)
                ? (int?)Convert.ToInt32(ddlKasa.SelectedValue) : (int?)null;
            int? renkId = renkGerek && !string.IsNullOrEmpty(ddlRenk.SelectedValue)
                ? (int?)Convert.ToInt32(ddlRenk.SelectedValue) : (int?)null;
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                bool kasaOk = !kasaGerek || rKasa == kasaId;
                bool renkOk = !renkGerek || rRenk == renkId;
                if (kasaOk && renkOk)
                {
                    eslesen = row;
                    break;
            ...
            {
                ViewState["VaryantId"] = null;
                if (kasaGerek && renkGerek)
                    lblStok.Text = "Lütfen kasa çapı ve renk seçin.";
                else if (kasaGerek)
                    lblStok.Text = "Lütfen kasa çapı seçin.";
                else if (renkGerek)
                    lblStok.Text = "Lütfen renk seçin.";
                else
                    lblStok.Text = "";
                ltrRenkSwatch.Text = "";
                ltrFiyat.Text = _temelFiyat.ToString("N2") + " TL";
            ...
            UygulaVaryantSatir(eslesen);
        }

        private void UygulaVaryantSatir(DataRow row)
        {
            _temelFiyat = ViewState["TemelFiyat"] != null ? Convert.ToDecimal(ViewState["TemelFiyat"]) : 0m;
            int varyantId = Convert.ToInt32(row["ID"]);
            int stok = Convert.ToInt32(row["StokAdedi"]);
            decimal fark = Convert.ToDecimal(row["FiyatFarki"]);
            ...
            string renkKodu = row["RenkKodu"]?.ToString()?.Trim() ?? "";
            if (!string.IsNullOrEmpty(renkKodu))
            {
                if (!renkKodu.StartsWith("#")) renkKodu = "#" + renkKodu;
                ltrRenkSwatch.Text = string.Format(
                    "<span class='d-inline-block mt-1' style='width:22px;height:22px;border-radius:50%;background:{0};border:1px solid #ccc;' title='Seçili renk'></span>",
                    System.Web.HttpUtility.HtmlAttributeEncode(renkKodu));
            }
            else
                ltrRenkSwatch.Text = "";
        }
        ...
        protected void btnSepeteEkle_Click(object sender, EventArgs e)
        {
            ...
            {
                lblMesaj.Text = lblStok.Text;
                if (string.IsNullOrWhiteSpace(lblMesaj.Text))
                    lblMesaj.Text = "Lütfen renk veya kasa çapı seçin.";
