...
FROM Musteri WHERE ID=@id", b.Baglanti))
        ...
        public static DemoCheckoutAdres CheckoutAdresiAl(int musteriId, int? adresId = null)
        {
            ...
            using (var b2 = new BaglantiBilgileri())
            using (var cmd = new SqlCommand(sql, b2.Baglanti))
            {
                cmd.Parameters.AddWithValue("@mid", musteriId);
            ...
            return MusteriAnaKayitAdresi(musteriId, email);
        }

        static DemoCheckoutAdres MusteriAnaKayitAdresi(int musteriId, string email)
        {
            using (var b = new BaglantiBilgileri())
            using (var cmd = new SqlCommand(
                "SELECT Ad, Soyad, Telefon, Adres, Il, Ilce, PostaKodu FROM Musteri WHERE ID=@id", b.Baglanti))
            {
                cmd.Parameters.AddWithValue("@id", musteriId);
                if (cmd.Connection.State == ConnectionState.Closed) cmd.Connection.Open();
                using (var rdr = cmd.ExecuteReader())
