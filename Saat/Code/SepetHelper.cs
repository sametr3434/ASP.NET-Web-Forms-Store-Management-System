...
namespace Saat.App_Code
{
    ...
    public static class SepetHelper
    {
            ...
            int urunId, int? varyantId, int adet, string paraBirimi, decimal birimFiyat)
        {
            ...
            {
                using (var cmd = new SqlCommand(
                    @"INSERT INTO Sepet(OturumId,MusteriId,UrunId,UrunVaryantId,Adet,EklenmeTarihi,BirimFiyat)
                      VALUES(@oid,@mid,@uid,@vid,@adet,@et,@bf)", b.Baglanti))
                {
            ...
            IlgiTakipHelper.SepetKaydet(ctx, urunId);
            return null;
            }
        }
        ...
        public static DataTable Listele(HttpContext ctx)
        {
            ...
            using (var b = new BaglantiBilgileri())
            {
            string sql = @"SELECT s.ID, s.UrunId, s.UrunVaryantId, s.Adet, s.BirimFiyat,
                ISNULL(s.ParaBirimi, N'TRY') AS ParaBirimi,
                u.UrunAdi, u.AnaResim, ISNULL(m.MarkaAdi, '') AS MarkaAdi,
                kb.kasano AS KasaBoyutu, rk.RenkAdi
                FROM Sepet s
                INNER JOIN Urun u ON s.UrunId = u.ID
                LEFT JOIN Marka m ON u.MarkaId = m.ID
                LEFT JOIN UrunVaryant uv ON s.UrunVaryantId = uv.ID
                LEFT JOIN kasaboyutu kb ON uv.KasaId = kb.ID
                LEFT JOIN Renk rk ON uv.RenkId = rk.ID
                WHERE ";
            if (MusteriAuth.GirisMi(ctx.Session))
            {
