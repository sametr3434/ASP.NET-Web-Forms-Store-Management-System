    ...
    public static class OneriMotoru
    {
        public static DataTable OnerileriGetir(HttpContext ctx, int adet, int? haricUrunId = null)
        {
            ...
            try
            {
                ...
                if (markaId.HasValue && markaId.Value > 0)
                {
                    ...
                    sonuc = VitrinUrunler(adet - dt.Rows.Count, haricUrunId, ctx, dt);
                    if (sonuc.Rows.Count > 0) return sonuc;
                }
            }
            catch
            {
            ...
            sonuc = VitrinUrunler(adet, haricUrunId, ctx, null);
            if (sonuc.Rows.Count > 0) return sonuc;

            // Son çare: sadece açık ürün hariç, stok şartı olmadan vitrin (demo katalog küçükse)
            return VitrinUrunler(adet, haricUrunId, ctx, null, stokZorunlu: false);
        }
        ...
        private static DataTable MarkadanUrunler(int markaId, int adet, int? haricUrunId, HttpContext ctx)
        {
            ...
            var sql = new StringBuilder(@"
                ...
                WHERE u.Aktif = 1 AND " + StokVarSql("u") + @" AND u.MarkaId = @MarkaId ");
        ...
        private static DataTable VitrinUrunler(int adet, int? haricUrunId, HttpContext ctx, DataTable mevcut, bool stokZorunlu = true)
        {
            using (var b = new BaglantiBilgileri())
            {
            var sql = new StringBuilder(@"
        ...
        /// <summary>Ana stok veya varyant stok varsa ürün önerilebilir.</summary>
        private static string StokVarSql(string uAlias)
        {
            return "(" + uAlias + ".StokAdedi > 0 OR EXISTS (SELECT 1 FROM UrunVaryant v WHERE v.UrunId = " + uAlias + ".ID AND v.StokAdedi > 0))";
        }
